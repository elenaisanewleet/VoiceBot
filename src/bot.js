...
const HELP =
  ...
  'На компьютере всё то же самое. Если микрофон окно не получит — так бывает ' +
  'в настольных клиентах, — там же появится кнопка «Открыть в браузере»: ' +
  'наговорите текст во вкладке и вернитесь с ним через буфер обмена.\n\n' +
...
export async function onMessage(msg) {
  ...
  if (text.startsWith('/')) {
    ...
    await tg.sendMessage(chatId, HELP.replaceAll('%USERNAME%', info.username), {
      reply_markup: { inline_keyboard: [[micButton(userId)]] },
    })
    return
  }
