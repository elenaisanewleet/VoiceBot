...
const HELP =
  'Я превращаю вашу речь в грамотный текст для любого чата.\n\n' +
  '1. Нажмите «🎙 Говорить» — откроется окно записи.\n' +
  '2. Говорите. Текст пишется на глазах, пунктуация и падежи — на мне.\n' +
  '3. «Отправить» — Telegram спросит, в какой чат, выберите собеседника.\n' +
  '4. Текст подставится в его чат — останется нажать на него.\n\n' +
  ...
  'Почему запись открывается отсюда, а не прямо в переписке: Mini App, ' +
  'открытый из панели в чужом чате, не получает от Telegram данных о том, кто ' +
  'его открыл. Без этого нельзя ни распознать, ни отправить от вашего имени.'
...
export async function onInlineQuery(q) {
  ...
  await tg.answerInlineQuery(q.id, results, {
    // Кнопка ведёт в чат с ботом, а не открывает окно записи прямо здесь.
    // Так пришлось: Mini App, открытый из панели в чате, не получает от
    // Telegram данных запуска — ни кто открыл, ни куда отправлять, — а без
    // них нельзя ни распознать (некому доверять), ни отправить.
    button: { text: '🎙 Записать голосом', start_parameter: 'speak' },
    cache_time: 0,
    is_personal: true,
  })
}
...
export async function onMessage(msg) {
  ...
  if (text.startsWith('/')) {
    ...
    await tg.sendMessage(chatId, HELP.replaceAll('%USERNAME%', info.username), {
      reply_markup: { inline_keyboard: [[micButton()]] },
    })
    return
  }
